...
Le segnalazioni di crash, o degli arresti anomali, sono una parte molto importante della risoluzione di problemi con il tuo gioco o server. Contengono molte informazioni riguardanti il crash, e ti possono aiutare a trovarne la causa.
...
Le segnalazioni di crash sono molto lunghe, e possono causare confusione nella lettura. Tuttavia, contengono molte informazioni riguardanti il crash, e ti possono aiutare a trovarne la causa.

Per questa guida, useremo [questa segnalazione di crash](/assets/players/crash-report-example.log).

:::details Mostra segnalazione di crash

<<< @/public/assets/players/crash-report-example.log

:::

### Sezioni delle segnalazioni di crash {#crash-report-sections}

Le segnalazioni di crash sono composte di varie sezioni, ciascuna indicata da un'intestazione:

- `---- Minecraft Crash Report ----`, il riassunto della segnalazione. Questa sezione contiene l'errore principale che ha causato il crash, l'orario al quale è avvenuto, e la stack trace pertinente. Questa è la sezione più importante della segnalazione del crash poiché la stack trace potrebbe solitamente contenere riferimenti alla mod che ha causato il crash.
...
### Comprendere la segnalazione di crash {#breaking-down-the-crash-report}

Ora che sappiamo cos'è ciascuna sezione della segnalazione di crash, possiamo iniziare ad analizzare la segnalazione di crash e trovare la causa del crash.

Usando l'esempio citato sopra, possiamo analizzare la segnalazione di crash e trovare la causa del crash, incluse le mod che l'hanno causato.

La stack trace nella sezione `---- Minecraft Crash Report ----` è la parte più importante in questo caso, poiché contiene l'errore principale che ha causato il crash.
...
Data la quantità di mod menzionata nello stack trace, può essere difficile puntare il dito, ma la prima cosa da fare è cercare la mod che ha causato il crash.

In questo caso, la mod che ha causato il crash è `snownee`, poiché è la prima mod citata nella stack trace.
...
## Crash legati ai mixin {#mixin-crashes}
...
I metodi mixin conterranno `mod-id$handlerName` nello stack trace, dove `mod-id` è l'ID della mod, e `handlerName` è il nome del gestore del mixin.
...
## Cosa fare con le segnalazioni di crash {#what-to-do-with-crash-reports}

La migliore cosa da fare con le segnalazioni di crash è caricarle su un paste site, e poi condividere il link con l'autore della mod, o tramite il suo issue tracker, o attraverso qualche altro mezzo di comunicazione (Discord ecc...).
